...
--exec uSp_ConsultaXMLDataCREaEjemplos
-- =============================================
CREATE PROCEDURE [dbo].[uSp_ConsultaXMLDataCREaEjemplos]
...
AS 
	...
	-- interfering with SELECT statements.
        ...
        DECLARE @folioBanco NVARCHAR(30)

		DECLARE @id BIGINT
  
		SET @id=(SELECT TOP 1 id FROM  tblIngresaXmlEjemplo WHERE enviado=0)      

		SET @xmlDataCREa=(SELECT  xmlEjemplo FROM  tblIngresaXmlEjemplo WHERE id=@id)

		UPDATE tblIngresaXmlEjemplo SET enviado=1 WHERE id=@id  --decomentar cuando se termine la prueba

        SET @folioBanco =( SELECT  
