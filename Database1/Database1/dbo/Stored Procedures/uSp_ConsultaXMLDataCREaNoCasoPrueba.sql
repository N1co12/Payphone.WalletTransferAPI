...
--exec uSp_ConsultaXMLDataCREaNoCasoPrueba '201511007389593'
--Select * from tblDataCREAPruebas where noCaso='201511007389593'
--update tblDataCREA set enviadoPresto=0 where id=81
-- =============================================
CREATE PROCEDURE [dbo].[uSp_ConsultaXMLDataCREaNoCasoPrueba]
...
AS 
	...
	-- interfering with SELECT statements.
        ...
        SET @maxId =(SELECT MAX(id) FROM   tblDataCREAPruebas WHERE noCaso = @noCaso AND codigo in(100,140))        
        
        SET @xmlDataCREa = ( SELECT xmlDataCREa
                             FROM   dbo.tblDataCREAPruebas
                             WHERE noCaso = @noCaso AND id=@maxId  --AND status=1 AND enviadoPresto=0
                           )
        ...
        SELECT  Container.ContainerCol.value('tipoParticipante[1]',
                ...
                Container.ContainerCol.value('(/DataCREa/DatosDomicilioCREa/DatosDomicilio/telefono/tipo_tel)[1]',
                                              'nvarchar(50)') tipo_tel1,
                Container.ContainerCol.value('(/DataCREa/DatosDomicilioCREa/DatosDomicilio/telefono/telefono)[1]',
                                              'nvarchar(50)') telefono1,
                Container.ContainerCol.value('(/DataCREa/DatosDomicilioCREa/DatosDomicilio/telefono/ext)[1]',
                                              'nvarchar(50)') ext1
  ...
  -- --                  --ACOCApertura  16
        ...
        SELECT  Container.ContainerCol.value('idSLA[1]',
                                             'int') AS idSLA ,
                Container.ContainerCol.value('idPasoPresto[1]', 
                                             'nvarchar(50)') AS idPasoPresto ,
                Container.ContainerCol.value('nombrePasoPresto[1]',
                                             'nvarchar(50)') AS nombrePasoPresto ,
                Container.ContainerCol.value('fechaInicio[1]',
                                             'nvarchar(50)') AS fechaInicio ,
                Container.ContainerCol.value('fechaFin[1]',
                                             'nvarchar(50)') AS fechaFin ,
                Container.ContainerCol.value('idUsuarioPresto[1]',
                                             'int') AS idUsuarioPresto ,
                Container.ContainerCol.value('nombreUsuarioPresto[1]',
                                             'nvarchar(50)') AS nombreUsuarioPresto ,
                Container.ContainerCol.value('resultadoTransaccion[1]',
                                             'nvarchar(50)') AS resultadoTransaccion ,
