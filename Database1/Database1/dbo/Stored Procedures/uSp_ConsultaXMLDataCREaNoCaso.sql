...
--exec uSp_ConsultaXMLDataCREaNoCaso '201603007933384'
--Select * from tblDataCREA where noCaso='201603007933384' order by 1
--update tblDataCREA set enviadoPresto=0  where id=32033
-- =============================================
CREATE PROCEDURE [dbo].[uSp_ConsultaXMLDataCREaNoCaso] 
...
AS 
	...
	-- interfering with SELECT statements.
		...
		IF(Exists(SELECT top 1 * FROM    dbPrestoCibergestionBanorte.dbo.tblIngreso_DatosGenerales WHERE folioBanco =@noCaso))
		  BEGIN
        ...
        SET @xmlDataCREa = ( SELECT top 1 xmlDataCREa
                             FROM   tblDataCREA
...
SELECT	distinct	
	...
	isNull(Container2.value('tipo_tel[1]', 'nvarchar(50)'), '') AS tipo_tel ,
    isNull(Container2.value('telefono[1]', 'nvarchar(30)'), '') AS telefono ,
    isNull(Container2.value('ext[1]', 'nvarchar(30)'), '') AS ext
FROM @xmlDataCREa.nodes('//DataCREa//DatosDomicilioCREa//DatosDomicilio') AS Container ( ContainerCol )
	outer apply @xmlDataCREa.nodes('//DataCREa//DatosDomicilioCREa//DatosDomicilio//telefono//Telefono') as nodo01(Container2)	
    ...
    --    FROM    @xmlDataCREa.nodes('//DataCREa//DatosDomicilioCREa//DatosDomicilio') AS Container ( ContainerCol ),  
				--@xmlDataCREa.nodes('//DataCREa//DatosDomicilioCREa//DatosDomicilio//telefono//Telefono') AS Container2 ( ContainerCol2 )
        ...
        SELECT  Container.ContainerCol.value('noGarantia[1]', 'int') AS noGarantia ,
                ...
                Container.ContainerCol.value('lote[1]', 'nchar(10)') AS lote ,
                Container.ContainerCol.value('manzana[1]', 'nchar(10)') AS manzana ,
                Container.ContainerCol.value('m2construccion[1]',
                                             'decimal(10, 2)') AS m2construccion ,
                Container.ContainerCol.value('m2terreno[1]', 'decimal(10, 2)') AS m2terreno ,
                Container.ContainerCol.value('ruv[1]', 'nvarchar(30)') AS ruv ,
                Container.ContainerCol.value('latitud[1]', 'decimal(10, 2)') AS latitud ,
                Container.ContainerCol.value('longitud[1]', 'decimal(10, 2)') AS longitud ,
        ...
        SELECT  Container.ContainerCol.value('importe[1]', 'nvarchar(50)') AS importe ,
				...
				Container.ContainerCol.value('decreto[1]', 'nvarchar(20)') AS decreto ,
   ...
   --                  --fillersconAva 10
        ...
        SELECT  Container.ContainerCol.value('noCtaChequesClientes[1]', 'nvarchar(20)') AS noCtaChequesClientes ,
				...
				, [ValorDestructible]		 = cast(0 as decimal(18,2)) --Container.ContainerCol.value('valorDestructible[1]', 'nvarchar(20)')
				, [IdPaqueteSeguros]		 = Container.ContainerCol.value('idPaqueteSeguros[1]', 'int')
				, [IdSeguroVida]			 = Container.ContainerCol.value('idSeguroVida[1]', 'int')
				, [AseguradoraVida]			 = Container.ContainerCol.value('aseguradoraVida[1]', 'nvarchar(500)')
				, [IdSeguroDanos]			 = Container.ContainerCol.value('idSeguroDanios[1]', 'int')
				, [AseguradoraDanos]		 = Container.ContainerCol.value('aseguradoraDanos[1]', 'nvarchar(500)')
				, [IdSeguroDesempleo]		 = Container.ContainerCol.value('idSeguroDesempleo[1]', 'int')
				, [AseguradoraDesempleo]	 = Container.ContainerCol.value('aseguradoraDesempleo[1]', 'nvarchar(500)')
				, [IdDomiciliacion]			 = Container.ContainerCol.value('idDomiciliacion[1]', 'int')
				, [DomiciliacionCuentaClabe] = Container.ContainerCol.value('domiciliacionCuentaCLABE[1]', 'nvarchar(20)')
				, [IdBanco]					 = Container.ContainerCol.value('idBanco[1]', 'int')
